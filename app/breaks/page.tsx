...
const TIERS = [
  {
    ...
    desc: "3 packs · basis parallel kaarten mogelijk",
    accent: "#6b7280",
    cards: [
  ...
  },
  {
    ...
    desc: "3 packs · refractors & foils gegarandeerd",
    accent: "#3b82f6",
    cards: [
  ...
  },
  {
    ...
    desc: "3 packs · ultra rares & special illustrations",
    accent: "#f59e0b",
    cards: [
...
export default function BreaksPage() {
  ...
  const selectTier = (t: typeof TIERS[0]) => {
    setTier(t);
    setSelected([]);
    setPhase("packs");
  };
  ...
  const flipCard = (i: number) => {
    ...
    if (next.every(Boolean)) {
      const cards = openedPacks[openingIdx];
      setAllCards(prev => [...prev, ...cards]);
    }
  };

  const nextPack = () => {
    if (openingIdx + 1 >= openedPacks.length) {
      setPhase("done");
    } else {
      setOpeningIdx(i => i + 1);
      setFlipped(Array(CARDS_PER_PACK).fill(false));
          ...
          <>
            <div style={{ textAlign: "center", marginBottom: "48px" }}>
              ...
              <p style={{ color: "#6b7280", fontSize: "14px", maxWidth: "400px", margin: "0 auto" }}>
                Kies een tier, selecteer 3 packs uit 20, en open ze live op je scherm.
              </p>
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: "20px" }}>
              {TIERS.map(t => (
                <button key={t.id} onClick={() => selectTier(t)}
                  style={{ background: "#111", border: `1px solid #1f1f1f`, borderRadius: "16px", padding: "32px 24px", textAlign: "left", cursor: "pointer", transition: "all 0.2s", color: "#fff" }}
                  onMouseEnter={e => { (e.currentTarget as HTMLElement).style.borderColor = t.accent; (e.currentTarget as HTMLElement).style.transform = "translateY(-4px)"; }}
                  onMouseLeave={e => { (e.currentTarget as HTMLElement).style.borderColor = "#1f1f1f"; (e.currentTarget as HTMLElement).style.transform = "none"; }}>
                  ...
                  <div style={{ background: t.accent, color: "#fff", borderRadius: "8px", padding: "10px 20px", fontSize: "12px", fontWeight: 800, letterSpacing: "0.05em", textTransform: "uppercase", display: "inline-block" }}>
                    Selecteer →
                  </div>
                </button>
              ))}
          ...
          <>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "32px", flexWrap: "wrap", gap: "12px" }}>
              ...
              <div style={{ display: "flex", gap: "12px", alignItems: "center" }}>
                ...
                <button onClick={() => setPhase("tiers")}
                  style={{ background: "transparent", border: "1px solid #333", color: "#9ca3af", borderRadius: "8px", padding: "8px 16px", fontSize: "12px", cursor: "pointer" }}>
                  ← Terug
                </button>
                <button onClick={openPacks} disabled={selected.length < PICKS}
                  style={{ background: selected.length >= PICKS ? tier.accent : "#1f1f1f", color: selected.length >= PICKS ? "#fff" : "#4b5563", border: "none", borderRadius: "8px", padding: "10px 24px", fontSize: "12px", fontWeight: 800, cursor: selected.length >= PICKS ? "pointer" : "not-allowed", letterSpacing: "0.05em", textTransform: "uppercase", transition: "all 0.2s" }}>
                  Open packs →
                </button>
              </div>
            </div>
              ...
              {Array.from({ length: PACK_COUNT }).map((_, i) => {
                ...
                return (
                  <button key={i} onClick={() => togglePack(i)}
                    style={{ aspectRatio: "2/3", borderRadius: "12px", border: `2px solid ${isSel ? tier.accent : "#1f1f1f"}`, background: isSel ? `${tier.accent}22` : "#111", cursor: selected.length >= PICKS && !isSel ? "not-allowed" : "pointer", position: "relative", transition: "all 0.15s", transform: isSel ? "scale(1.05)" : "scale(1)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: "8px" }}>
        ...
        {phase === "opening" && tier && currentPack.length > 0 && (
          <>
            <div style={{ textAlign: "center", marginBottom: "32px" }}>
              ...
              <h2 style={{ fontSize: "28px", fontWeight: 900, letterSpacing: "-0.5px", margin: "0 0 8px" }}>Klik op de kaarten om te openen</h2>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: "16px", marginBottom: "32px" }}>
              {currentPack.map((card, i) => (
                <div key={i} onClick={() => !flipped[i] && flipCard(i)}
                  style={{ aspectRatio: "2/3", borderRadius: "14px", cursor: flipped[i] ? "default" : "pointer", perspective: "1000px", position: "relative" }}>
                  {!flipped[i] ? (
                    <div style={{ width: "100%", height: "100%", borderRadius: "14px", background: "linear-gradient(135deg, #1a1a2e, #16213e)", border: "1px solid #2d2d4e", display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column", gap: "8px", transition: "transform 0.1s" }}
                    ...
                    <div style={{ width: "100%", height: "100%", borderRadius: "14px", background: `linear-gradient(135deg, var(--from), var(--to))`, border: `1px solid ${card.rarityColor}44`, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "12px", gap: "8px", animation: "fadeIn 0.3s ease" }}
